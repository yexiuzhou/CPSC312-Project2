...
% ---------------------------------
requirementPhrase(['total', 'credit', 'requirement'], T) :- creditCount(T).
requirementPhrase(['science', 'credit', 'requirement'], T) :- sciCount(T).
requirementPhrase(['upper', 'level', 'credit', 'requirement'], T) :- upperCount(T).
requirementPhrase(['upper', 'level', 'science', 'credit', 'requirement'], T) :- upperScieCount(T).
requirementPhrase(['art', 'credit', 'requirement'], T) :- artCount(T).
requirementPhrase(['breadth', 'requirement'], T) :- breadth(T).
requirementPhrase(['communications', 'requirement'], T) :- communicationsRequirement(T,R).
requirementPhrase(['lab', 'requirement'], T) :- lab_requirements(T,R).

% total credit count, science credit count, upper level credit count, upper level science credit count
% art credit, breadth requirement, communications requirement, lab requirement, upper level cs,
...
artCount(T) :-
    onlyArtsCreditCounter(T, ArtsCourses, ArtCredit),
    12 @=< ArtCredit,
    ArtCredit @=< 18.
...
% onlyMissXBreadth(T,List) is true if all type of courses in the list are present in the transcript
onlyMissXBreadth(T,[]).
onlyMissXBreadth(T,[H|R]) :-
    course(C, department, H),
    member(C,T),
    onlyMissXBreadth(T,R).

% ---------------------------------
% Communications and Lab Requirements Helpers
% ---------------------------------
...
% ---------------------------------
% havePreReqs(C, T) is true if Transcript T contains prereqs for Course C
havePreReqs(C, T) :-
    course(C,_,_),
    preReqs(P, C),
    subset(P, T).

% missingCourses(C, T, Ans) is true if Ans is a list of courses that are not in Transcript T but are prereqs for Course C
...
% cpen courses
% ---------------------------------
course(cpen221, number, 210).
course(cpen221, department, cpen).
course(cpen221, faculty, appliedScience).
course(cpen221, credits, 4).
course(cpen221, name, "cpen221").

% ---------------------------------
% econ courses
% ---------------------------------
course(econ325, number, 325).
course(econ325, department, econ).
course(econ325, faculty, arts).
course(econ325, credits, 3).
course(econ325, name, "econ325").

course(econ327, number, 327).
course(econ327, department, econ).
course(econ327, faculty, arts).
course(econ327, credits, 3).
course(econ327, name, "econ327").
...
course(math152, number, 152).
course(math152, department, math).
course(math152, faculty, science).
course(math152, credits, 3).
course(math152, name, "math152").
...
course(math217, number, 217).
course(math217, department, math).
course(math217, faculty, science).
course(math217, credits, 3).
course(math217, name, "math217").

course(math220, number, 220).
course(math220, department, math).
course(math220, faculty, science).
course(math220, credits, 3).
course(math220, name, "math220").

course(math226, number, 226).
course(math226, department, math).
course(math226, faculty, science).
course(math226, credits, 3).
course(math226, name, "math226").


% ---------------------------------
% 300 level
% ---------------------------------
course(math318, number, 318).
course(math318, department, math).
course(math318, faculty, science).
course(math318, credits, 3).
course(math318, name, "math318").
